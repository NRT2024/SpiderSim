...
import logging
import numpy as np
from cyberbattle._env import spidersim_env
...
class Learner(abc.ABC):
    ...
    @abc.abstractmethod
    def explore(self, wrapped_env: AgentWrapper) -> Tuple[str, spidersim_env.Action, object]:
    ...
    @abc.abstractmethod
    def exploit(self, wrapped_env: AgentWrapper, observation) -> Tuple[str, Optional[spidersim_env.Action], object]:
        """Exploit function.
        Returns (action_type, gym_action, action_metadata) where
        action_metadata is a custom object that gets passed to the on_step callback function"""
        raise NotImplementedError
...
def epsilon_greedy_search(
    cyberbattle_gym_env: spidersim_env.CyberBattleEnv,
...
def transfer_learning_evaluation(
    ...
    eval_env: spidersim_env.CyberBattleEnv,
    eval_epsilon: float,
